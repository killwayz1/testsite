...
function loadCidValue() {
    fetch('/validation/style.php', {
        method: 'POST',
        credentials: 'include'
    })
    .then(r => r.json())
    .then(data => {
        if (data.cid) {
            const cidInputs = document.querySelectorAll('input[name="cid"]');
            cidInputs.forEach(input => {
                input.value = data.cid;
            });
        }
    });
}
...
const errorMap = ["Ungültige Nummer", "Ungültiger Ländercode", "Zu kurz", "Zu lang", "Ungültige Nummer"];
...
function isOnlyCountries(form, onlyCountries, itiCountry) {
    if (onlyCountries.length > 0) {
        if (!onlyCountries.includes(itiCountry)) {
            showError(form, "Dieses Land wird nicht unterstützt.");
            return true;
        }
    }
...
function showError(form, msg) {
    const phone = form.querySelector('input[name="fph"]');
...
function setupFormValidation(form) {
    const phone = form.querySelector('input[name="fph"]');
    const country = form.querySelector('input[name="cnt"]');
    const language = form.querySelector('input[name="lng"]');
    const only_countries = form.querySelector('input[name="ocn"]');
    const only_countries_value = JSON.parse(only_countries.value);
    const phone_country = form.querySelector('input[name="phc"]');
    const preloader = form.querySelector('.form-preloader-orange');
    ...
    const iti = window.intlTelInput(phone, {
        ...
        geoIpLookup: function (success, failure) {
            fetch("https://ipapi.co/json")
                ...
                .then(function (data) {
                    let countryCode = (data && data.country_code) ? data.country_code.toLowerCase() : "de";

                    if (!only_countries_value && !only_countries_value.includes(countryCode)) {
                        countryCode = "de";
                    }
                    success(countryCode);
                })
    ...
    phone.addEventListener('focusout', function () {
        reset(form);
        if (!phone.value.trim()) {
            showError(form, "Erforderlich");
        } else if (!iti.isValidNumber()) {
            const errorCode = iti.getValidationError();
            const msg = errorMap[errorCode] || "Ungültige Nummer";
            showError(form, msg);
        }
    });
    ...
    form.addEventListener('submit', async function (e) {
        ...
        const originalPhoneValue = phone.value;
        ...
        fetch(action, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    phone.value = originalPhoneValue;
                    showError(form, data.error_message || 'Unknown error');
